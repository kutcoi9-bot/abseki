...
$latitude_lokasi  = '-3.0879994';
$longitude_lokasi = '119.8502572';

// --- Audio notifikasi ---
$audio_success = '<audio autoplay style="display:none;"><source src="../sw-content/audio/absen_berhasil.mp3" type="audio/mpeg"></audio>';
$audio_fail    = '<audio autoplay style="display:none;"><source src="../sw-content/audio/absen_gagal.mp3" type="audio/mpeg"></audio>';

/**
 * Output response konsisten:
 * - success/... -> absensi benar-benar berhasil tersimpan/terupdate di database
 * - gagal/...   -> absensi gagal
 */
function response_absensi($status, $message, $audio = '') {
  echo $status . '/' . $message . $audio;
  exit;
}

/**
 * Cek koneksi database untuk mendeteksi gangguan jaringan / database.
 * Jika koneksi putus, query biasanya gagal. Ini adalah pengecekan server-side
 * yang paling relevan untuk memastikan aplikasi tahu koneksi ke DB aktif.
 */
function koneksi_aktif($connection) {
...
case 'absen-in':
  ...
  if (!koneksi_aktif($connection)) {
    response_absensi('gagal', 'Ulangi Absensi.! Koneksi jaringan/database tidak aktif.', $audio_fail);
  }

  if (empty($_POST['qrcode'])) {
    $error[] = 'Ulangi Absensi.! Barcode/Qr Code tidak boleh kosong.';
  } else {
    $qrcode = anti_injection(trim($_POST['qrcode']));
  }

  if (!empty($error)) {
    foreach ($error as $values) {
      response_absensi('gagal', $values, $audio_fail);
    }
  }
  ...
  if ($result_siswa === false) {
    response_absensi('gagal', 'Ulangi Absensi.! Gangguan koneksi saat membaca data siswa.', $audio_fail);
  }

  if ($result_siswa->num_rows <= 0) {
    response_absensi('gagal', 'Ulangi Absensi.! NISN/Barcode tidak terbaca atau siswa tidak ditemukan.', $audio_fail);
  }
  ...
  if ($result_waktu === false) {
    response_absensi('gagal', 'Ulangi Absensi.! Gangguan koneksi saat membaca jadwal absensi.', $audio_fail);
  }

  if ($result_waktu->num_rows <= 0) {
    response_absensi('gagal', 'Ulangi Absensi.! Hari ini tidak ada jadwal/jam sekolah.', $audio_fail);
  }
  ...
  if ($result_absen === false) {
    response_absensi('gagal', 'Ulangi Absensi.! Gangguan koneksi saat memeriksa data absensi.', $audio_fail);
  }
  ...
  if (!$result_absen->num_rows > 0) {
    ...
    if ($connection->query($add) === false) {
      response_absensi('gagal', 'Ulangi Absensi.! Data absensi gagal disimpan.', $audio_fail);
    }

    /**
     * Validasi berhasil = data siswa benar-benar sudah terbaca dan terekam di aplikasi
     */
    $validasi_simpan = "SELECT absen_id FROM absen WHERE tanggal='$date' AND user_id='$data_siswa[user_id]' AND absen_in='$time_absen' LIMIT 1";
    $result_validasi = $connection->query($validasi_simpan);

    if ($result_validasi === false || $result_validasi->num_rows <= 0) {
      response_absensi('gagal', 'Ulangi Absensi.! Data belum berhasil terekam di aplikasi.', $audio_fail);
    }
    ...
    response_absensi(
      'success',
      'Terima Kasih.! "'.$data_siswa['nama_lengkap'].'", absensi Anda berhasil pada Tanggal '.tanggal_ind($date).' dan Jam : '.$time_absen.' Selamat Belajar!',
      $audio_success
    );
  }

  /**
   * Jika sudah ada absen hari ini, anggap berhasil hanya jika data memang sudah tersimpan
   */
  response_absensi(
    'success',
    'Terima Kasih.! "'.$data_siswa['nama_lengkap'].'", absensi Anda sudah tercatat pada Tanggal '.tanggal_ind($date).' dan Jam : '.$time_absen.'.',
    $audio_success
  );
break;
...
case 'absen-out':
  ...
  if ($result_absen === false) {
    response_absensi('gagal', 'Ulangi Absensi.! Gangguan koneksi saat memeriksa data absensi pulang.', $audio_fail);
  }

  if ($result_absen->num_rows <= 0) {
    response_absensi('gagal', 'Ulangi Absensi.! Sebelumnya siswa '.$data_siswa['nama_lengkap'].' belum pernah absen masuk.', $audio_fail);
  }
  ...
  if ($data_absensi['absen_out'] == '00:00:00') {
    ...
    if ($connection->query($update) === false) {
      response_absensi('gagal', 'Ulangi Absensi.! Data absensi pulang gagal disimpan.', $audio_fail);
    }

    $validasi_pulang = "SELECT absen_id FROM absen
                        WHERE tanggal='$date'
                        AND user_id='$data_siswa[user_id]'
                        AND absen_out='$time_absen'
                        LIMIT 1";
    $result_validasi = $connection->query($validasi_pulang);

    if ($result_validasi === false || $result_validasi->num_rows <= 0) {
      response_absensi('gagal', 'Ulangi Absensi.! Data absensi pulang belum berhasil terekam di aplikasi.', $audio_fail);
    }
    ...
    response_absensi(
      'success',
      'Terima Kasih.! Anda berhasil absen pulang pada Tanggal '.tanggal_ind($date).' dan Jam : '.$time.'. Sampai ketemu besok "'.$data_siswa['nama_lengkap'].'"!',
      $audio_success
    );
  }
  ...
  response_absensi(
    'success',
    'Terima Kasih.! Anda sudah absen pulang pada Tanggal '.tanggal_ind($date).' dan Jam : '.$time.'.',
    $audio_success
  );
break;
