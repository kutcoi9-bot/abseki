...
|--------------------------------------------------------------------------
| batch1_min_students :
|   jumlah minimal siswa agar batch 1 langsung terkirim otomatis
|   contoh:
|   1 = langsung kirim saat 1 siswa absen
|   3 = kirim saat 3 siswa absen
|   5 = kirim saat 5 siswa absen
|
| masuk_cutoff :
|   jam penutupan sesi absensi masuk
|
| pulang_extra_minutes :
|   tambahan menit setelah jam_pulang untuk menutup sesi absensi pulang
|--------------------------------------------------------------------------
*/
$WA_BATCH_CONFIG = array(
  'batch1_min_students'  => 1,
...
|--------------------------------------------------------------------------
| GROUP CONFIG
| Urutan sesuai aplikasi / tabel kelas
|--------------------------------------------------------------------------
*/
...
function processClassNotificationBatches($connection, $kelas_id, $nama_kelas, $tipe, $tanggal, $current_time, $jadwal, $whatsapp_tipe, $sender, $link, $token, $config){
  ...
  |--------------------------------------------------------------------------
  | BATCH 1
  | - otomatis kirim saat jumlah siswa sudah mencapai threshold
  | - atau saat sesi ditutup walau belum mencapai threshold
  |--------------------------------------------------------------------------
  */
  if(
  ...
  |--------------------------------------------------------------------------
  | BATCH 2
  | - kirim sisa siswa saat sesi ditutup
  |--------------------------------------------------------------------------
  */
  if(
...
if (empty($error)){
  ...
  if ($result_siswa->num_rows > 0){
    ...
    if($result_waktu->num_rows > 0){
      ...
      // sebelum 10:00 tetap dianggap absensi masuk
      if($time < '10:00:00'){
